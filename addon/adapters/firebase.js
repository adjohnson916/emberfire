  ...
  */
  updateRecord: function(store, typeClass, snapshot) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      Ember.RSVP.hashSettled({record: recordPromise}).then(function (recordSettled) {
        ...
        var relationshipsPromise = Ember.RSVP.hashSettled({relationships: savedRelationships});

        relationshipsPromise.then(function(relationshipsSettled) {
          var rejected = Ember.A(relationshipsSettled.relationships.value).filterBy('state', 'rejected');
          if (recordSettled.record.state === 'rejected') {
            rejected.push(recordSettled.record);
          }
